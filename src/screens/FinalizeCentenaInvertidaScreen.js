...
const FinalizeCentenaInvertidaScreen = ({ route }) => {
    ...
    // Função para randomizar os números
    const randomizeNumbers = (numbers) => {
        const randomizedNumbers = numbers.map(num => {
            const numArray = num.toString().split('');
            const shuffledNum = numArray.sort(() => Math.random() - 0.5).join('');
            return shuffledNum;
        });
        return randomizedNumbers;
    };

    const randomizedNumbers = randomizeNumbers(numbers);
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            <View style={styles.container}>
                ...
                <View style={localStyles.summaryContainer}>
                    ...
                    <View style={localStyles.numbersContainer}>
                        <Text style={localStyles.summaryText}>
                            Números: {randomizedNumbers.join(', ')}
                        </Text>
                    </View>
                    <FlatList
                        data={betValues}
...
export default FinalizeCentenaInvertidaScreen;
