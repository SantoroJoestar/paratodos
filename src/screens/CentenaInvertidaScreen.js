...
const CentenaInvertidaScreen = ({ navigation }) => {
  ...
  const handleNext = () => {
    if (numbers.length === 0) {
      ...
      const invertedNumbers = numbers.map(num => {
        return num.toString().split('').reverse().join('');
      });
      navigation.navigate('PrizesCentenaInvertidaScreen', { numbers: invertedNumbers });
    }
  };
...
export default CentenaInvertidaScreen;
