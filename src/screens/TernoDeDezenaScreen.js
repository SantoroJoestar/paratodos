...
const TernoDeDezenaScreen = ({ navigation }) => {
  ...
  const handleNext = () => {
    if (numbers.length === 0) {
      ...
      navigation.navigate('PrizesTernoDeDezenaScreen', { numbers });
    }
  };
  ...
  const handleInputChange = (text) => {
    ...
    if (sanitizedText.length === 7) { // Espera o formato "XX-XX"
      addNumber(text);
    } else if (sanitizedText.length === 2) {
      setNumber(text + '-');
    } else if (sanitizedText.length === 4) {
        setNumber(text + '-');
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        maxLength={9} // Ajustado para incluir o traço
        value={number}
...
export default TernoDeDezenaScreen;